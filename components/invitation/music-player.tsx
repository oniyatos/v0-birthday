...
export function MusicPlayer() {
	...
	useEffect(() => {
		...
		audioRef.current.src = '/obgtl.mp3'
		audioRef.current.loop = false // tắt loop mặc định

		const audio = audioRef.current

		// Khi load xong → nhảy tới đoạn start
		audio.addEventListener('loadedmetadata', () => {
			audio.currentTime = startTime
		})

		// Khi chạy tới end → quay lại start (loop đoạn)
	...
	const startTime = 10 // giây bắt đầu
	const endTime = 250 // giây kết thúc
			...
			<motion.button
				...
				style={{
					background: isPlaying
						...
						: 'linear-gradient(135deg, rgba(255, 151, 227, 0.74), rgba(255, 255, 255, 0.8))',
					boxShadow: isPlaying
					...
					{isPlaying ? (
						<div className='relative'>
							...
							<motion.div
								...
								style={{
									background:
										'conic-gradient(from 0deg, #1f1f1f, #3a3a3a, #1f1f1f, #3a3a3a, #1f1f1f)',
